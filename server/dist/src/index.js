...
io.on("connection", (socket) => {
    ...
    socket.on("create-room", (playerName) => {
        const roomCode = (0, gameState_1.generateRoomCode)();
        const room = (0, gameState_1.createRoom)(roomCode, socket.id, playerName);
    ...
    socket.on("start-game", (roomCode) => {
        ...
        const state = (0, gameState_1.startGame)(room);
    ...
    socket.on("play-card", (roomCode) => {
        try {
            ...
            const result = (0, gameState_1.playCard)(state, socket.id);
            if (!result) {
            ...
            let pileWinnerId = null;
            let pileWinnerName = null;
            ...
            const winner = (0, gameState_1.checkWinner)(state);
            if (winner) {
    ...
    socket.on("slap", (roomCode) => {
        try {
            ...
            console.log(`Card counts: ${state.room.players.map(p => `${p.name}(${p.hand.length})`).join(", ")}`);
            const result = (0, gameState_1.handleSlap)(state, socket.id);
            if (result.valid) {
